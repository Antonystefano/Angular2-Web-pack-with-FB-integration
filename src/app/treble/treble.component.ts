...
declare var StartStaff : any;
declare var timeOut : any;
declare var resumeGame : any;
declare var stopGame : any;
declare var g_point : any;
declare var gCorrect1 : any;
declare var gWrong1 : any;

@Component({
  // The selector is what angular internally uses
  // for `document.querySelectorAll(selector)` in our index.html
  // where, in this case, selector is the string 'home'
  selector: 'treble',  // <home></home>
  // We need to tell Angular's Dependency Injection which providers are in our app.
  providers: [
    Title,
  ],
  styleUrls : ['./treble.component.css'],
  template : `
    ...
    <div id="holder1"></div>
    <div id="buttons" style="width : 560px; margin-left:40px;"></div>      
  `
})
export class TrebleComponent {
  ...
  ngDoCheck()  {
    if (g_point != this.score){
      this.score = g_point;
    }
    if (gCorrect1 != this.nCorrect){
      this.nCorrect = gCorrect1;
    }
    if (gWrong1 != this.nWrong){
      this.nWrong = gWrong1;
    }
  }
  ...
  tickerFunc(tick){
    if (this.isOpen){
      if (this.second > 0) this.second --;
        ...
        timeOut();
        this.router.navigate(['./home/treble/fbshare']);
        this.second = 90;
      }
      this.ticks = tick
    }
